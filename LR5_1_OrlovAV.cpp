...
void task(Array& array) {
	main_task(array);

	double first_negative = 0;

	for (int i = 0; i < array.size(); i++) {
		if (first_negative) break;
		if (array[i] < 0) {
			first_negative = array[i];
		}
	}
	double fifth_element = array.size() >= 5 ? array[5] : 0;
	double task_delimiter = (first_negative + fifth_element) / 2;
	double deleimiter = task_delimiter ? task_delimiter : 1;
	for (int i = 0; i < array.size(); i++) {
		array[i] /= deleimiter;
	}
